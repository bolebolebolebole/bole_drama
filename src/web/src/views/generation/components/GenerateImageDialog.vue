...
:placeholder="$t('imageDialog.negativePromptPlaceholder')"
      ...
      <el-form-item :label="$t('imageDialog.referenceImages')">
        <ReferenceImageManager 
          v-model="form.reference_images" 
          :max-images="5"
        />
      </el-form-item>
      ...
      <el-form-item :label="$t('imageDialog.imageSize')">
        <el-select v-model="form.size" :placeholder="$t('imageDialog.selectSize')">
          <el-option :label="`2048x1152 (${$t('imageDialog.landscape')})`" value="2048x1152" />
          <el-option :label="`1152x2048 (${$t('imageDialog.portrait')})`" value="1152x2048" />
        </el-select>
      </el-form-item>
...
const form = reactive<GenerateImageRequest>({  drama_id: props.dramaId || '',
  scene_id: undefined,
  prompt: '',
  negative_prompt: '',
  reference_images: [],
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (form.reference_images && form.reference_images.length > 0) {
        params.reference_images = form.reference_images
      }
      ...
      const globalStyle = localStorage.getItem('global_style') || 'anime'
      params.style = globalStyle
