  ...
  <div class="professional-editor">
    ...
    <div class="editor-main">
      ...
      <div class="timeline-area">
        <VideoTimelineEditor ref="timelineEditorRef" v-if="storyboards.length > 0" :scenes="storyboards"
          :episode-id="episodeId" :drama-id="dramaId.toString()" :assets="videoAssets"
              ...
              <div class="image-generation-section">
                ...
                <div class="prompt-section">
                  ...
                  <el-input v-model="currentFramePrompt" type="textarea" :rows="8"
                    :placeholder="$t('editor.promptPlaceholder')" @blur="saveFramePrompt" />
                </div>

                 <!-- 参考图（仅本次生成生效） -->
                 <div class="reference-images-section" v-if="referenceSlots.length > 0">
                   <div class="section-label">
                     参考图
                     <el-button v-if="hasAnyReferenceOverride" size="small" text @click="resetAllReferenceOverrides"
                       style="margin-left: 10px;">
                       清空替换
                     </el-button>
                   </div>
                   <div class="reference-images-grid">
                     <div v-for="slot in referenceSlots" :key="slot.key" class="reference-image-item"
                       :class="{ 'disabled': slot.disabled }"
                       @dblclick="slot.disabled ? toggleDisableReference(slot.key, false) : null">
                       <div class="image-container">
                         <el-image :src="getReferenceImageUrl(slot)" fit="cover" class="reference-image-thumbnail"
                           preview-teleported />
                         
                         <!-- 禁用遮罩 -->
                         <div v-if="slot.disabled" class="disabled-overlay">
                           <span class="disabled-text">已禁用</span>
                         </div>
                         
                         <!-- X 按钮（悬停显示） -->
                         <div v-else class="close-button" @click="toggleDisableReference(slot.key, true)">
                           <el-icon><Close /></el-icon>
                         </div>
                       </div>
                       <div class="reference-image-name">{{ slot.name }}</div>
                       <div class="reference-image-actions">
                         <el-upload v-if="!slot.custom" class="reference-image-upload" :action="uploadAction" :headers="uploadHeaders"
                           :show-file-list="false" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp"
                           :before-upload="beforeReferenceImageUpload"
                           :on-success="(response) => handleReferenceImageUploadSuccess(response, slot.key)"
                           :on-error="handleReferenceImageUploadError">
                           <el-button size="small" :icon="Upload">替换</el-button>
                         </el-upload>
                         <el-button v-if="hasReferenceOverride(slot.key) && !slot.custom" size="small" text
                           @click="resetReferenceOverride(slot.key)">
                           恢复
                         </el-button>
                         <el-button v-if="slot.custom" size="small" text type="danger"
                           @click="removeCustomReference(slot.key)">
                           删除
                         </el-button>
                       </div>
                     </div>
                 ...
                 <div class="generation-controls">
                   ...
                   <!-- 添加自定义参考图按钮 -->
                   <el-button type="primary" size="default" :icon="Plus" @click="showCustomImageUpload = true">
                     添加参考图
                   </el-button>
                 </div>
           ...
           <el-tab-pane :label="$t('video.videoGeneration')" name="video">
             <div class="tab-content" v-if="currentStoryboard">
               <div class="video-generation-section">
                <!-- 提示词区域（对齐“镜头图片”页的交互结构） -->
                <div class="prompt-section">
                  <div class="section-label">{{ $t('editor.prompt') }}</div>
                  <el-input
                    ...
                    placeholder="请输入视频生成提示词（将直接用于视频生成）"
                    @blur="saveVideoPrompt"
                    style="width: 100%; --el-textarea-inner-height: 380px !important;"
                  />
                </div>
                    ...
                    <div v-show="selectedVideoFrameType === 'first'" class="image-scroll-container"
                      ...
                      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                        <div class="reference-grid"
                          style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; max-width: 600px; flex: 1;">
                          <div
                        ...
                        <el-upload class="video-image-upload" :action="uploadAction" :headers="uploadHeaders"
                          :show-file-list="false" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp"
                          :before-upload="beforeVideoImageUpload"
                          :on-success="(response) => handleVideoImageUploadSuccess(response, 'first')"
                          :on-error="handleVideoImageUploadError">
                          <el-button type="primary" :icon="Upload" size="default">上传图片</el-button>
                        </el-upload>
                      </div>
                      <el-empty
                    ...
                    <div v-show="selectedVideoFrameType === 'key'" class="image-scroll-container"
                      ...
                      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                        ...
                        <el-upload class="video-image-upload" :action="uploadAction" :headers="uploadHeaders"
                          :show-file-list="false" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp"
                          :before-upload="beforeVideoImageUpload"
                          :on-success="(response) => handleVideoImageUploadSuccess(response, 'key')"
                          :on-error="handleVideoImageUploadError">
                          <el-button type="primary" :icon="Upload" size="default">上传图片</el-button>
                        </el-upload>
                      </div>
                    ...
                    <div v-show="selectedVideoFrameType === 'last'" class="image-scroll-container"
                      ...
                      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                        ...
                        <el-upload class="video-image-upload" :action="uploadAction" :headers="uploadHeaders"
                          :show-file-list="false" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp"
                          :before-upload="beforeVideoImageUpload"
                          :on-success="(response) => handleVideoImageUploadSuccess(response, 'last')"
                          :on-error="handleVideoImageUploadError">
                          <el-button type="primary" :icon="Upload" size="default">上传图片</el-button>
                        </el-upload>
                      </div>
                    ...
                    <div v-show="selectedVideoFrameType === 'panel'" class="image-scroll-container"
                      ...
                      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                        ...
                        <el-upload class="video-image-upload" :action="uploadAction" :headers="uploadHeaders"
                          :show-file-list="false" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp"
                          :before-upload="beforeVideoImageUpload"
                          :on-success="(response) => handleVideoImageUploadSuccess(response, 'panel')"
                          :on-error="handleVideoImageUploadError">
                          <el-button type="primary" :icon="Upload" size="default">上传图片</el-button>
                        </el-upload>
                      </div>
                    ...
                    <div v-show="selectedVideoFrameType === 'action'" class="image-scroll-container"
                      ...
                      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                        ...
                        <el-upload class="video-image-upload" :action="uploadAction" :headers="uploadHeaders"
                          :show-file-list="false" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp"
                          :before-upload="beforeVideoImageUpload"
                          :on-success="(response) => handleVideoImageUploadSuccess(response, 'action')"
                          :on-error="handleVideoImageUploadError">
                          <el-button type="primary" :icon="Upload" size="default">上传图片</el-button>
                        </el-upload>
                      </div>
                  ...
                  <div class="image-grid"
                    ...
                    <div v-for="video in generatedVideos" :key="video.id" class="image-item video-item"
                      ...
                      <div class="video-thumbnail" v-if="video.video_url"
                         ...
                         @mouseenter="onVideoThumbEnter"
                         @mouseleave="onVideoThumbLeave"
                         @click="playVideo(video)">
                        <video :src="getVideoProxyUrl(video.video_url)" preload="metadata"
    ...
    <el-dialog v-model="showVideoPreview" title="视频预览" width="800px" :close-on-click-modal="true" destroy-on-close>
      <div class="video-preview-container" v-if="previewVideo">
        <video v-if="previewVideo.video_url" :src="getVideoProxyUrl(previewVideo.video_url)" controls autoplay
    ...
    >
      ...
      <template #footer>
        <el-button @click="showCustomImageUpload = false">取消</el-button>
        <el-button type="primary" @click="handleCustomUploadConfirm" :disabled="!selectedCustomFile">
          确定
        </el-button>
      </template>
    </el-dialog>
...
import {
  ...
  Timer, Calendar, Clock, Loading, WarningFilled, Delete, InfoFilled, UploadFilled
} from '@element-plus/icons-vue'
import { dramaAPI } from '@/api/drama'
import { generateFramePrompt, getStoryboardFramePrompts, updateFramePrompt, updateReferenceOverrides, type FrameType } from '@/api/frame'
...
// 图片生成相关状态
  ...
  // 帧提示词存储：按 '分镜ID_帧类型' 的键来存储，确保不同分镜的提示词不互相覆盖
...
const selectedReferenceMode = ref<'none' | 'single' | 'first_last' | 'multiple'>('single')
const selectedVideoFrameType = ref<FrameType>('key')
...
const storyboardImagesCache = new Map<number, ImageGeneration[]>()
const storyboardVideosCache = new Map<number, VideoGeneration[]>()
...
const loadVideoModels = async () => {
  try {
    ...
    const allModels = activeConfigs.flatMap(config => {
      const rawModels = Array.isArray(config.model)
        ? config.model
    ...
    // 构建模型能力列表
    const capabilityList = Array.from(modelMap.keys()).map(modelName => {
    ...
    // 没有可用配置时，提供一个可用的默认模型，避免“模型为空”导致无法生成视频
    if (capabilityList.length === 0) {
      const fallbackModel = 'doubao-seedance-1-5-pro-251215'
      const capability = defaultModelCapabilities[fallbackModel] || {
        supportSingleImage: true,
        supportMultipleImages: false,
        supportFirstLastFrame: true,
        supportTextOnly: true,
      ...
      videoModelCapabilities.value = [{ id: fallbackModel, name: fallbackModel, ...capability }]
      return
    }

    videoModelCapabilities.value = capabilityList
  } catch (error: any) {
...
const getVideoPromptStorageKey = (storyboardId: number | undefined) => {
  if (!storyboardId) return null
  return `video_prompt_${storyboardId}`
}

const getDefaultVideoPrompt = (sb: Storyboard | null) => {
  if (!sb) return ''
  return (sb.video_prompt || sb.action || sb.description || '').trim()
}
...
// 切换镜头时加载该镜头的参考图替换
watch(() => currentStoryboard.value?.id, (newId) => {
  if (currentStoryboard.value?.reference_overrides) {
    referenceImageOverrides.value = { ...currentStoryboard.value.reference_overrides }
  } else {
    referenceImageOverrides.value = {}
  }
}, { immediate: true })
...
watch(selectedFrameType, async (newType) => {
  ...
  // 切换帧类型时，停止该镜头之前的轮询，避免旧结果覆盖新帧类型
  stopPolling(storyboardId)

  // 设置切换标志，防止watch(currentFramePrompt)错误保存
  isSwitchingFrameType.value = true

  // 从 framePrompts 对象中加载该帧类型的提示词（使用分镜ID前缀）
  const framePromptsKey = `${storyboardId}_${newType}`
  currentFramePrompt.value = framePrompts.value[framePromptsKey] || ''

  // 从 sessionStorage 中加载该帧类型之前的提示词（如果framePrompts中没有）
  if (!currentFramePrompt.value) {
    const storageKey = `frame_prompt_${storyboardId}_${newType}`
    const stored = sessionStorage.getItem(storageKey)
    if (stored) {
      currentFramePrompt.value = stored
      framePrompts.value[framePromptsKey] = stored
    }
  }
...
watch(currentStoryboard, async (newStoryboard) => {
  if (!newStoryboard) {
    ...
    videoPromptDraft.value = ''
    return
  }

  // 先检查缓存，如果有缓存立即显示（提升响应速度）
  if (storyboardImagesCache.has(newStoryboard.id)) {
    generatedImages.value = storyboardImagesCache.get(newStoryboard.id) || []
    loadingImages.value = false
  }
  if (storyboardVideosCache.has(newStoryboard.id)) {
    generatedVideos.value = storyboardVideosCache.get(newStoryboard.id) || []
    loadingVideos.value = false
  }

  // 初始化/恢复视频提示词（允许用户手动编辑，不强制写回后端）
  const videoPromptKey = getVideoPromptStorageKey(newStoryboard.id)
  const storedVideoPrompt = videoPromptKey ? sessionStorage.getItem(videoPromptKey) : null
  if (storedVideoPrompt && storedVideoPrompt.trim()) {
    videoPromptDraft.value = storedVideoPrompt
  } else {
    videoPromptDraft.value = getDefaultVideoPrompt(newStoryboard)
  }

  // 设置切换标志
  isSwitchingFrameType.value = true

  // 清空之前的帧提示词缓存，避免不同分镜数据混淆
  framePrompts.value = {}
  ...
  // 加载当前帧类型的提示词（优先从数据库加载，sessionStorage作为备份）
  const storageKey = getPromptStorageKey(newStoryboard.id, selectedFrameType.value)
  const framePromptsKey = `${newStoryboard.id}_${selectedFrameType.value}`

  if (storageKey) {
    const stored = sessionStorage.getItem(storageKey)
    if (stored && stored.trim()) {
      // 如果sessionStorage中有值，优先使用（用户编辑过但未保存的情况）
      currentFramePrompt.value = stored
    } else {
      // 否则使用从数据库加载的提示词
      currentFramePrompt.value = framePrompts.value[framePromptsKey] || ''
...
// 监听视频提示词变化，自动保存到 sessionStorage
watch(videoPromptDraft, (newPrompt) => {
  if (!currentStoryboard.value) return
  const key = getVideoPromptStorageKey(currentStoryboard.value.id)
  if (!key) return

  const val = (newPrompt || '').trim()
  if (val) {
    sessionStorage.setItem(key, newPrompt)
  } else {
...
watch(activeTab, (newTab) => {
  if (newTab === 'video') {
    if (videoModelCapabilities.value.length > 0) {
      selectedVideoModel.value = 'doubao-seedance-1-5-pro-251215'
      selectedReferenceMode.value = 'single'
      selectedVideoFrameType.value = 'key'
    }
  }
})

// 监听视频模型列表加载完成，自动设置视频生成默认值（处理模型列表晚于标签页切换加载的情况）
watch(videoModelCapabilities, (newCapabilities) => {
  if (newCapabilities.length > 0 && activeTab.value === 'video') {
    selectedVideoModel.value = 'doubao-seedance-1-5-pro-251215'
    selectedReferenceMode.value = 'single'
    selectedVideoFrameType.value = 'key'
  }
...
const referenceSlots = computed<ReferenceImageSlot[]>(() => {
  ...
  if (sceneUrl) {
    ...
    slots.push({
      ...
      disabled: disabledReferenceImages.value.has(`scene:${sb.id}`)
    })
  }
  ...
  if (Array.isArray(storyboardCharacters) && storyboardCharacters.length > 0) {
    storyboardCharacters.forEach((char: any) => {
      ...
      slots.push({
        ...
        originalUrl: url,
        disabled: disabledReferenceImages.value.has(`character:${char.id}`)
      })
    })
  }
  ...
  if (Array.isArray(customReferenceImages.value)) {
    customReferenceImages.value.forEach((img, index) => {
      ...
      slots.push({
        ...
        originalUrl: url,
        custom: true,
        disabled: disabledReferenceImages.value.has(img.key)
      })
    })
  }
...
const hasAnyReferenceOverride = computed(() => Object.keys(referenceImageOverrides.value).length > 0)
const hasReferenceOverride = (key: string) => Boolean(referenceImageOverrides.value[key])
const getReferenceImageUrl = (slot: ReferenceImageSlot) => referenceImageOverrides.value[slot.key] || slot.originalUrl

const resetReferenceOverride = async (key: string) => {
  if (!referenceImageOverrides.value[key]) return
  const next = { ...referenceImageOverrides.value }
  delete next[key]
  referenceImageOverrides.value = next
  
  if (currentStoryboard.value) {
    try {
      await updateReferenceOverrides(currentStoryboard.value.id, referenceImageOverrides.value)
      ElMessage.success('已恢复原始参考图')
    } catch (error: any) {
...
const resetAllReferenceOverrides = async () => {
  referenceImageOverrides.value = {}
  
  if (currentStoryboard.value) {
    try {
      await updateReferenceOverrides(currentStoryboard.value.id, {})
      ElMessage.success('已清空所有替换')
    } catch (error: any) {
      ElMessage.error('保存失败：' + (error.message || '未知错误'))
    }
  }
...
const extractUploadedUrl = (response: any): string => response?.url || response?.data?.url || ''

const beforeReferenceImageUpload = (file: any) => {
...
const handleReferenceImageUploadSuccess = async (response: any, slotKey: string) => {
  const url = extractUploadedUrl(response)
  if (!url) {
    ElMessage.error('上传失败：未获取到图片地址')
    return
  }
  referenceImageOverrides.value = { ...referenceImageOverrides.value, [slotKey]: url }
...
const handleReferenceImageUploadError = () => {
  ElMessage.error('上传失败，请重试')
}

// 禁用/启用参考图
const toggleDisableReference = (key: string, disabled: boolean) => {
  if (disabled) {
    disabledReferenceImages.value.add(key)
    ElMessage.success('参考图已禁用')
  } else {
    disabledReferenceImages.value.delete(key)
    ElMessage.success('参考图已恢复')
  }
}

// 添加自定义参考图
const handleAddCustomReferenceImage = async (response: any) => {
  const url = extractUploadedUrl(response)
  if (!url) {
    ElMessage.error('上传失败：未获取到图片地址')
    return
  ...
  customReferenceImages.value.push({
    key: key,
    kind: 'custom',
    name: `自定义${customReferenceImages.value.length + 1}`,
    originalUrl: url,
    custom: true,
    disabled: false
  })
  ...
  showCustomImageUpload.value = false
}

// 删除自定义参考图
const removeCustomReference = (key: string) => {
  const index = customReferenceImages.value.findIndex(img => img.key === key)
  if (index === -1) return
  
  customReferenceImages.value.splice(index, 1)
  disabledReferenceImages.value.delete(key)
  ElMessage.success('参考图已删除')
}

// 处理自定义文件选择
const handleCustomFileChange = (file: any) => {
  selectedCustomFile.value = file.raw
}

// 确认上传自定义参考图
const handleCustomUploadConfirm = async () => {
  if (!selectedCustomFile.value) {
    ElMessage.warning('请选择文件')
    return
  }
  
  if (!beforeReferenceImageUpload(selectedCustomFile.value)) {
    return
  }
  
  const formData = new FormData()
  formData.append('file', selectedCustomFile.value)
  ...
  try {
    ...
    await handleAddCustomReferenceImage(data)
  } catch (error: any) {
    ElMessage.error(error.message || '上传失败')
  }
}

const beforeVideoImageUpload = (file: any) => {
  const allowedTypes = new Set(['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'])
  if (!allowedTypes.has(file.type)) {
    ElMessage.error('只支持图片格式 (jpg, png, gif, webp)')
    return false
...
const handleVideoImageUploadSuccess = async (response: any, frameType: string) => {
  const url = extractUploadedUrl(response)
  if (!url) {
    ElMessage.error('上传失败：未获取到图片地址')
    return
  ...
  try {
    const newImage = await imageAPI.create({
      ...
      image_url: url,
      status: 'completed',
      prompt: videoPromptDraft.value || currentStoryboard.value.video_prompt || currentStoryboard.value.action || currentStoryboard.value.description || ''
    })
...
const handleVideoImageUploadError = () => {
  ElMessage.error('上传失败，请重试')
}
...
const saveVideoPrompt = async () => {
  ...
  const val = (videoPromptDraft.value || '').trim()
  if (!val) return
  ...
  try {
    ...
    // Keep local state in sync so refresh/load reflects the saved prompt.
    currentStoryboard.value.video_prompt = val
    const key = getVideoPromptStorageKey(currentStoryboard.value.id)
...
const saveFramePrompt = async () => {
  ...
  const val = (currentFramePrompt.value || '').trim()
  if (!val) return

  try {
    await updateFramePrompt(
      currentStoryboard.value.id,
      selectedFrameType.value,
      { prompt: val }
    )

    const framePromptsKey = `${currentStoryboard.value.id}_${selectedFrameType.value}`
    framePrompts.value[framePromptsKey] = val
...
const extractFramePrompt = async () => {
  ...
  try {
    ...
    // 存储到对应帧类型的提示词中（使用分镜ID前缀确保隔离）
    const framePromptsKey = `${currentStoryboard.value.id}_${targetFrameType}`
    framePrompts.value[framePromptsKey] = extractedPrompt

    // 清空sessionStorage，因为数据已经在数据库中，下次加载会从数据库读取
    const storageKey = getPromptStorageKey(currentStoryboard.value.id, targetFrameType)
    if (storageKey) {
      sessionStorage.removeItem(storageKey)
    }
...
const loadFramePrompts = async (storyboardId: number) => {
  try {
    ...
    const framePromptsData = result.frame_prompts || []

    // 将数据库中的提示词填充到 framePrompts 对象中（使用分镜ID前缀）
    framePromptsData.forEach((record: any) => {
      const key = `${storyboardId}_${record.frame_type}`
      framePrompts.value[key] = record.prompt
    })
  } catch (error: any) {
...
const loadStoryboardImages = async (storyboardId: number, frameType?: string) => {
  // 只有当前镜头才显示loading状态
  const isCurrentStoryboard = currentStoryboard.value?.id === storyboardId
  if (isCurrentStoryboard) {
  ...
  try {
    ...
    // 更新缓存
    storyboardImagesCache.set(storyboardId, images)
    ...
    // 如果有进行中的任务，启动该镜头的轮询
    const hasPendingOrProcessing = images.some(
      img => img.status === 'pending' || img.status === 'processing'
    )
    if (hasPendingOrProcessing && frameType) {
      startPolling(storyboardId, frameType as FrameType)
    }
  } catch (error: any) {
...
// 启动图片状态轮询（按镜头ID管理）
const startPolling = (storyboardId: number, frameType: FrameType) => {
  // 如果该镜头已有轮询，先停止
  if (pollingTimers.has(storyboardId)) {
    clearInterval(pollingTimers.get(storyboardId))
  }

  // 记录该镜头的轮询帧类型
  pollingFrameTypes.set(storyboardId, frameType)
  ...
  const timer = setInterval(async () => {
    try {
      ...
      if (currentStoryboard.value?.id === storyboardId) {
        // 检查帧类型是否仍然匹配
        if (selectedFrameType.value === frameType) {
          generatedImages.value = images
        }
      }
      ...
      console.error(`轮询镜头${storyboardId}图片状态失败:`, error)
    }
  }, 3000) // 每3秒轮询一次

  pollingTimers.set(storyboardId, timer)
}

// 停止图片轮询
const stopPolling = (storyboardId?: number) => {
  if (storyboardId) {
    // 停止指定镜头的轮询
    const timer = pollingTimers.get(storyboardId)
    if (timer) {
      clearInterval(timer)
      pollingTimers.delete(storyboardId)
      pollingFrameTypes.delete(storyboardId)
    }
  } else {
    // 停止所有轮询（组件卸载时使用）
    pollingTimers.forEach(timer => clearInterval(timer))
    pollingTimers.clear()
    pollingFrameTypes.clear()
  }
}
...
const generateFrameImage = async () => {
  ...
  try {
    ...
    // 更新缓存
    const currentCache = storyboardImagesCache.get(storyboardId) || []
    storyboardImagesCache.set(storyboardId, [result, ...currentCache])
    ...
    // 启动该镜头的轮询
    startPolling(storyboardId, selectedFrameType.value)
  } catch (error: any) {
...
const generateVideo = async () => {
  ...
  const finalPrompt = (videoPromptDraft.value || currentStoryboard.value.video_prompt || currentStoryboard.value.action || currentStoryboard.value.description || '').trim()
  if (finalPrompt.length < 5) {
    ElMessage.warning('提示词太短，请补充后再生成')
    return
  }
  ...
  try {
    ...
    // 更新缓存
    const currentCache = storyboardVideosCache.get(storyboardId) || []
    storyboardVideosCache.set(storyboardId, [result, ...currentCache])
...
const loadStoryboardVideos = async (storyboardId: number) => {
  ...
  try {
    ...
    // 更新缓存
    storyboardVideosCache.set(storyboardId, videos)

    // 只有当前镜头才更新UI
    if (isCurrentStoryboard) {
      generatedVideos.value = videos
    }

    // 如果有进行中的任务，启动该镜头的轮询
    const hasPendingOrProcessing = videos.some(
...
// 启动视频状态轮询（按镜头ID管理）
const startVideoPolling = (storyboardId: number) => {
  // 如果该镜头已有轮询，先停止
  if (videoPollingTimers.has(storyboardId)) {
    clearInterval(videoPollingTimers.get(storyboardId))
  }
  ...
  const timer = setInterval(async () => {
    try {
      ...
      if (!hasPendingOrProcessing) {
        // 任务完成，停止该镜头的轮询
        stopVideoPolling(storyboardId)
      }
    } catch (error) {
      console.error(`轮询镜头${storyboardId}视频状态失败:`, error)
    }
  }, 5000) // 每5秒轮询一次

  videoPollingTimers.set(storyboardId, timer)
}

// 停止视频轮询
const stopVideoPolling = (storyboardId?: number) => {
  if (storyboardId) {
    // 停止指定镜头的轮询
    const timer = videoPollingTimers.get(storyboardId)
    if (timer) {
      clearInterval(timer)
      videoPollingTimers.delete(storyboardId)
    }
  } else {
    // 停止所有轮询（组件卸载时使用）
    videoPollingTimers.forEach(timer => clearInterval(timer))
...
const selectScene = async (sceneId: number) => {
  ...
  try {
    // Update storyboard's linked scene.
    await dramaAPI.updateStoryboard(currentStoryboard.value.id.toString(), {
      scene_id: sceneId
    })
...
const onVideoThumbEnter = (e: MouseEvent) => {
  const target = e.currentTarget as HTMLElement | null
  const overlay = target?.querySelector<HTMLElement>('.play-overlay')
  if (overlay) overlay.style.opacity = '1'
}

const onVideoThumbLeave = (e: MouseEvent) => {
  const target = e.currentTarget as HTMLElement | null
  const overlay = target?.querySelector<HTMLElement>('.play-overlay')
...
const handleAddStoryboard = async () => {
  ...
  if (currentStoryboardId.value) {
    const currentStoryboard = storyboards.value.find(s => s.id === currentStoryboardId.value)
    if (currentStoryboard) {
      insertAfter = currentStoryboard.storyboard_number
    }
  } else if (storyboards.value.length > 0) {
    // 如果有分镜但没选中，插入到最后
    insertAfter = storyboards.value[storyboards.value.length - 1].storyboard_number
  }
  
  try {
    const newStoryboard = await dramaAPI.insertStoryboard(episodeId.value, insertAfter, 5)
    
    ElMessage.success(`已在第${insertAfter}个分镜后添加新分镜`)
    ...
    // 自动选中新创建的分镜
    await nextTick()
    selectStoryboard(newStoryboard.id)
    
  } catch (error: any) {
    ElMessage.error('添加分镜失败：' + (error.message || '未知错误'))
    console.error('Failed to add storyboard:', error)
  }
}
...
.reference-images-section {
  ...
  .reference-image-item.disabled .reference-image-thumbnail {
