...
:description="$t('drama.management.noEpisodes')"
          ...
          <el-col :span="6" v-for="character in drama?.characters" :key="character.id">
            <el-card shadow="hover" class="character-card">
              ...
              <div class="character-actions">
                <el-button
                  size="small"
                  :disabled="isCharacterRegenerating(character.id)"
                ...
                <el-button
                  size="small"
                  type="primary"
                  :loading="isCharacterRegenerating(character.id)"
                  @click="regenerateCharacter(character)"
                >
                  重新生成
                </el-button>
              </div>
            ...
            <el-card shadow="hover" class="scene-card">
              ...
              <div class="scene-actions">
                ...
                <el-button
                  size="small"
                  type="primary"
                  :loading="isSceneRegenerating(scene.id)"
                  @click="regenerateScene(scene)"
                >
                  重新生成
                </el-button>
              </div>
    ...
    >
      <el-form :model="editCharacterForm" label-width="110px">
        <el-form-item :label="$t('character.name')">
          <el-input v-model="editCharacterForm.name" :placeholder="$t('character.name')" />
        </el-form-item>
        <el-form-item :label="$t('character.role')">
          <el-select v-model="editCharacterForm.role" :placeholder="$t('common.pleaseSelect')">
            <el-option label="Main" value="main" />
            <el-option label="Supporting" value="supporting" />
            <el-option label="Minor" value="minor" />
          </el-select>
        </el-form-item>
        <el-form-item label="形象提示词">
          <el-input
            v-model="editCharacterForm.appearance"
            type="textarea"
            :rows="4"
        ...
        <el-form-item :label="$t('character.personality')">
          <el-input
            v-model="editCharacterForm.personality"
            type="textarea"
            :rows="3"
      ...
      <template #footer>
        <el-button
          :disabled="editCharacterSaving || isCharacterRegenerating(editCharacterForm.id)"
          @click="editCharacterDialogVisible = false"
        >
          {{ $t('common.cancel') }}
        </el-button>
        <el-button
          type="primary"
          :loading="editCharacterSaving"
          :disabled="isCharacterRegenerating(editCharacterForm.id)"
          @click="saveEditedCharacter(false)"
        >
          保存
        </el-button>
        <el-button
          type="primary"
          plain
          :loading="editCharacterSaving || isCharacterRegenerating(editCharacterForm.id)"
          @click="saveEditedCharacter(true)"
        >
          保存并重新生成
        </el-button>
      </template>
    ...
    <!-- 编辑场景对话框 -->
    <el-dialog
      v-model="editSceneDialogVisible"
      title="编辑场景"
      width="640px"
      :close-on-click-modal="false"
    >
      <el-form :model="editSceneForm" label-width="110px">
        <el-form-item label="场景名称">
          <el-input v-model="editSceneForm.name" disabled placeholder="-" />
        </el-form-item>
        <el-form-item label="地点">
          <el-input v-model="editSceneForm.location" placeholder="例如：室内 / 城市街道 / 山林" />
        </el-form-item>
        <el-form-item label="时间">
          <el-input v-model="editSceneForm.time" placeholder="例如：清晨 / 夜晚 / 黄昏" />
        </el-form-item>
        <el-form-item label="提示词">
          <el-input
            v-model="editSceneForm.prompt"
            type="textarea"
            :rows="5"
            placeholder="用于生成场景图的提示词（prompt）"
          />
        </el-form-item>
      </el-form>
      ...
      <template #footer>
        <el-button
          :disabled="editSceneSaving || isSceneRegenerating(editSceneForm.id)"
          @click="editSceneDialogVisible = false"
        >
          {{ $t('common.cancel') }}
        </el-button>
        <el-button
          type="primary"
          :loading="editSceneSaving"
          :disabled="isSceneRegenerating(editSceneForm.id)"
          @click="saveEditedScene(false)"
        >
          保存
        </el-button>
        <el-button
          type="primary"
          plain
          :loading="editSceneSaving || isSceneRegenerating(editSceneForm.id)"
          @click="saveEditedScene(true)"
        >
          保存并重新生成
        </el-button>
      </template>
...
const editCharacterDialogVisible = ref(false)
...
const characterRegenerating = ref<Record<number, boolean>>({})

const editSceneDialogVisible = ref(false)
...
const editCharacter = (character: any) => {
  ...
  editCharacterDialogVisible.value = true
}

const isCharacterRegenerating = (characterId?: number) => {
  if (!characterId) return false
  return !!characterRegenerating.value[Number(characterId)]
}

const isSceneRegenerating = (sceneId?: number | string) => {
  if (!sceneId) return false
  return !!sceneRegenerating.value[Number(sceneId)]
}

const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms))

const pollImageUntilCompleted = async (imageGenerationId: number): Promise<ImageGeneration> => {
  const maxAttempts = 120
  const intervalMs = 1500

  for (let i = 0; i < maxAttempts; i++) {
    const data = await imageAPI.getImage(imageGenerationId)
...
// Fire-and-forget so multiple regenerations can run concurrently.
const regenerateCharacter = (character: any) => {
  const characterId = Number(character?.id)
  if (!characterId) {
    ElMessage.error('角色ID不存在，无法生成图片')
    return
  }
  if (isCharacterRegenerating(characterId)) return

  characterRegenerating.value = {
    ...characterRegenerating.value,
    [characterId]: true
  }
  ...
  void (async () => {
    try {
      const result = await characterLibraryAPI.generateCharacterImage(String(characterId))

      const generationId = Number(result?.image_generation?.id)
      if (!generationId) {
      ...
      ElMessage.success('图片生成任务已提交')
      const completed = await pollImageUntilCompleted(generationId)
      if (completed.image_url && drama.value?.characters) {
        const target = drama.value.characters.find((c: any) => Number(c.id) === characterId)
        if (target) target.image_url = completed.image_url
      }
      ...
      const next = { ...characterRegenerating.value }
      delete next[characterId]
      characterRegenerating.value = next
    }
  })()
}

// Fire-and-forget so multiple regenerations can run concurrently.
const regenerateScene = (scene: any, promptOverride?: string) => {
  const sceneId = Number(scene?.id)
  if (!sceneId) {
    ElMessage.error('场景ID不存在，无法生成图片')
    return
  }
  if (isSceneRegenerating(sceneId)) return

  const prompt = (promptOverride ?? scene?.prompt ?? '').trim()
  ...
  void (async () => {
    try {
      const result = await dramaAPI.generateSceneImage({
        scene_id: sceneId,
        prompt
      })
      ...
      if (completed.image_url) {
        const targetInList = scenes.value.find(s => Number(s.id) === sceneId)
        if (targetInList) targetInList.image_url = completed.image_url

        const targetInDrama = drama.value?.scenes?.find(s => Number(s.id) === sceneId)
        if (targetInDrama) targetInDrama.image_url = completed.image_url
      }
...
const saveEditedCharacter = async (regenerateAfterSave: boolean) => {
  if (!editCharacterForm.value.name.trim()) {
    ElMessage.warning('请输入角色名称')
    return
  }
  ...
  try {
    const payload = {
      ...
      role: editCharacterForm.value.role,
      appearance: editCharacterForm.value.appearance,
      personality: editCharacterForm.value.personality,
      description: editCharacterForm.value.description
    }

    await characterLibraryAPI.updateCharacter(editCharacterForm.value.id, payload)
    ElMessage.success('角色已保存')

    if (regenerateAfterSave) {
      regenerateCharacter({ id: editCharacterForm.value.id })
    }

    editCharacterDialogVisible.value = false
    if (!regenerateAfterSave) {
      await loadDramaData()
    }
  } catch (error: any) {
...
const editScene = (scene: any) => {
  ...
  editSceneDialogVisible.value = true
}

const saveEditedScene = async (regenerateAfterSave: boolean) => {
  const sceneId = Number(editSceneForm.value.id)
  if (!sceneId) {
    ElMessage.error('场景ID不存在，无法保存')
    return
  }
  ...
  try {
    const payload = {
      location: editSceneForm.value.location,
      time: editSceneForm.value.time,
      prompt: editSceneForm.value.prompt
    }

    await dramaAPI.updateScene(String(sceneId), payload)
    ElMessage.success('场景已保存')

    if (regenerateAfterSave) {
      regenerateScene({ id: sceneId, prompt: editSceneForm.value.prompt }, editSceneForm.value.prompt)
    }

    editSceneDialogVisible.value = false
    if (!regenerateAfterSave) {
      await loadDramaData()
    }
  } catch (error: any) {
...
.scene-preview :deep(.el-image__inner) {
  width: 100%;
  height: 100%;
  cursor: zoom-in;
...
.scene-card:hover .scene-preview :deep(.el-image__inner) {
...
.character-preview :deep(.el-image__inner) {
  width: 100%;
  height: 100%;
  cursor: zoom-in;
  transition: filter var(--transition-normal);
...
.character-card:hover .character-preview :deep(.el-image__inner) {
  filter: brightness(1.05);
}
