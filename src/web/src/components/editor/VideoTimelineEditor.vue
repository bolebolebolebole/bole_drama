        ...
        <div class="media-grid">
          ...
          >
            <div class="media-thumbnail" @click="previewScene(scene)">
              <video :src="getVideoProxyUrl(scene.video_url)" />
            ...
            >
              <div class="clip-content">
                <div class="clip-thumbnail">
                  <video :src="getVideoProxyUrl(clip.video_url)" />
                </div>
                <div class="clip-info">
      ...
      <div class="download-path-container">
        <el-form label-width="100px">
          ...
          <el-form-item label="保存格式">
            <el-select v-model="downloadFormat" placeholder="选择视频格式">
...
type PositionedClip = { position: number; duration: number }
...
const currentPreviewUrl = computed(() => {
  ...
  const url = clip?.video_url || timelineClips.value[0]?.video_url || ''
  return getVideoProxyUrl(url)
})
...
const previewScene = (scene: Scene) => {
  if (previewPlayer.value) {
    previewPlayer.value.src = getVideoProxyUrl(scene.video_url)
...
const switchToClip = async (clip: TimelineClip) => {
  ...
  previewPlayer.value.src = getVideoProxyUrl(clip.video_url)
...
// 片段样式计算
const getClipStyle = (clip: PositionedClip) => {
  return {
...
const addClipToTimeline = async (scene: Scene, insertAtPosition?: number) => {
  if (!scene.video_url) {
    ElMessage.warning('该镜头暂无视频，无法加入时间线')
    return
  }
...
const seekToTime = (time: number) => {
  ...
  if (clip && previewPlayer.value) {
    // 切换视频源（如果需要）
    const proxiedUrl = getVideoProxyUrl(clip.video_url)
    if (previewPlayer.value.src !== proxiedUrl) {
      previewPlayer.value.src = proxiedUrl
    }
...
const handleKeyPress = (event: KeyboardEvent) => {
  // 如果在输入框/可编辑区域中，或处于输入法组合输入中，不处理快捷键
...
const selectDownloadPath = () => {
  // 这里可以实现文件夹选择逻辑
  // 由于浏览器安全限制，实际项目中可能需要使用文件系统API或其他方式
  // 这里暂时使用默认路径
  downloadPath.value = `${window.location.origin}/downloads`
  ElMessage.info('已选择默认路径')
}
...
const confirmDownload = async () => {
  ...
  try {
    // 按场景下载视频
    for (const clip of timelineClips.value) {
      await downloadVideoByScene(clip)
    }
    
    loadingMessage.close()
    ElMessage.success(`已成功下载 ${timelineClips.value.length} 个视频片段`)
  } catch (error: any) {
    loadingMessage.close()
...
// 按场景下载视频
const downloadVideoByScene = async (clip: TimelineClip) => {
  const sceneNumber = clip.storyboard_number
  const fileName = `场景${sceneNumber}.${downloadFormat.value}`
  const filePath = `${downloadPath.value}/场景${sceneNumber}/${fileName}`
  
  // 创建场景文件夹（实际项目中可能需要后端API支持）
  console.log(`创建场景文件夹: ${downloadPath.value}/场景${sceneNumber}`)
  
  // 下载视频（实际项目中可能需要处理跨域等问题）
  console.log(`下载视频: ${clip.video_url} 到 ${filePath}`)
  
  // 模拟下载过程
  return new Promise<void>((resolve) => {
    setTimeout(() => {
      console.log(`视频下载完成: 场景${sceneNumber}`)
      resolve()
    }, 1000)
  })
}
