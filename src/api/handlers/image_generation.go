...
func (h *ImageGenerationHandler) CreateImageGeneration(c *gin.Context) {
	...
	imageGen, err := h.imageService.CreateImageGeneration(req.StoryboardID, req.FrameType, req.ImageURL, req.Prompt)
	if err != nil {
		h.log.Errorw("Failed to create image generation", "error", err)
		response.InternalError(c, err.Error())
		return
	}
