...
func (h *FramePromptHandler) UpdateFramePrompt(c *gin.Context) {
	storyboardID := c.Param("id")
	...
	err := h.framePromptService.UpdateFramePrompt(storyboardID, frameType, req.Prompt)
	if err != nil {
		h.log.Errorw("Failed to update frame prompt", "error", err)
		response.InternalError(c, err.Error())
		return
	}
