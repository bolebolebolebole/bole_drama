...
func serveFileNoCache(filePath string) gin.HandlerFunc {
...
func serveDirFileNoCache(baseDir string) gin.HandlerFunc {
	return func(c *gin.Context) {
...
func SetupRouter(cfg *config.Config, db *gorm.DB, log *logger.Logger, localStorage interface{}) *gin.Engine {
	...
	videoProxyHandler := handlers2.NewVideoProxyHandler(log)
	...
	api := r.Group("/api/v1")
	{
		...
		episodes := api.Group("/episodes")
		{
			...
			episodes.POST("/:episode_id/storyboards/insert", storyboardHandler.InsertStoryboard)
		...
		images := api.Group("/images")
		{
			...
			images.POST("/create", imageGenHandler.CreateImageGeneration)
		...
		videos := api.Group("/videos")
		{
			...
			videos.GET("/proxy", videoProxyHandler.ProxyVideo)
			videos.GET("/proxy/range", videoProxyHandler.ProxyVideoRange)
		}
		...
		storyboards := api.Group("/storyboards")
		{
			...
			storyboards.PUT("/:id/reference-overrides", storyboardHandler.UpdateReferenceOverrides)
			storyboards.POST("/:id/frame-prompt", framePromptHandler.GenerateFramePrompt)
			storyboards.PUT("/:id/frame-prompt", framePromptHandler.UpdateFramePrompt)
	...
	// 服务前端构建产物
	r.GET("/assets/*filepath", serveDirFileNoCache("./web/dist/assets"))
	r.HEAD("/assets/*filepath", func(c *gin.Context) { noCacheHeaders(c); c.Status(http.StatusOK) })
	r.GET("/ffmpeg/*filepath", serveDirFileNoCache("./web/dist/ffmpeg"))
	r.HEAD("/ffmpeg/*filepath", func(c *gin.Context) { noCacheHeaders(c); c.Status(http.StatusOK) })
	...
	r.NoRoute(func(c *gin.Context) {
		...
		// Avoid stale index.html being cached across deployments.
		serveFileNoCache("./web/dist/index.html")(c)
	})
