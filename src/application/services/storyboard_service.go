...
// GetStoryboardWithFullData 获取分镜及其所有关联数据
func (s *StoryboardService) GetStoryboardWithFullData(id uint) (*models.Storyboard, error) {
	var storyboard models.Storyboard
	if err := s.db.Preload("Episode", "Background", "Characters").
		First(&storyboard, id).Error; err != nil {
...
// CopyStoryboard 复制分镜（创建新的分镜，复制所有数据）
func (s *StoryboardService) CopyStoryboard(sourceID uint, episodeID uint) (*models.Storyboard, error) {
	...
	if err := s.db.Preload("Episode", "Background", "Characters").
		First(&sourceSB, sourceID).Error; err != nil {
		return nil, fmt.Errorf("source storyboard not found: %w", err)
	}

	// 2. 查询同episode的所有分镜，按storyboard_number排序
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get all storyboards: %w", err)
	}
	...
	if sourceIndex == -1 {
		return nil, fmt.Errorf("source storyboard not found in list")
	}

	// 4. 确定插入位置（源分镜之后）
	var insertPosition = sourceIndex + 1

	// 5. 计算新的分镜序号（当前分镜之后的所有分镜都+1）
	newStoryboardNumber := sourceSB.StoryboardNumber
	for i := sourceIndex + 1; i < len(allStoryboards); i++ {
		if i >= insertPosition {
			newStoryboardNumber++
		}
	}
	...
	s.log.Infow("Storyboard copied",
		"source_id", sourceID,
		"new_id", newStoryboard.ID,
		"new_number", newStoryboardNumber)

	return &newStoryboard, nil
}

// CopyStoryboards 批量复制分镜（用于批量操作）
func (s *StoryboardService) CopyStoryboards(sourceIDs []uint, episodeID uint) ([]*models.Storyboard, error) {
	var newStoryboards []*models.Storyboard
	for _, sourceID := range sourceIDs {
		sb, err := s.CopyStoryboard(sourceID, episodeID)
		if err != nil {
			return nil, err
		}
		newStoryboards = append(newStoryboards, sb)
	}
	return newStoryboards, nil
}

// UpdateReferenceOverrides 更新参考图替换
func (s *StoryboardService) UpdateReferenceOverrides(storyboardID string, overrides map[string]interface{}) error {
	// 查找分镜
	var storyboard models.Storyboard
	if err := s.db.First(&storyboard, storyboardID).Error; err != nil {
		return fmt.Errorf("storyboard not found: %w", err)
	...
	if err != nil {
		return fmt.Errorf("failed to marshal reference overrides: %w", err)
	}

	// 更新参考图替换
	if err := s.db.Model(&storyboard).Update("reference_overrides", string(jsonData)).Error; err != nil {
		return fmt.Errorf("failed to update reference overrides: %w", err)
	}

	s.log.Infow("Reference overrides updated", "storyboard_id", storyboardID)
	return nil
}

// InsertStoryboard 在指定位置插入新分镜
func (s *StoryboardService) InsertStoryboard(episodeIDStr string, insertAfter, duration int) (*models.Storyboard, error) {
	...
	// 1. 将插入位置之后的所有分镜编号+1
	if err := tx.Model(&models.Storyboard{}).
		Where("episode_id = ? AND storyboard_number > ?", episodeID, insertAfter).
		UpdateColumn("storyboard_number", gorm.Expr("storyboard_number + 1")).Error; err != nil {
		tx.Rollback()
		return nil, fmt.Errorf("failed to update storyboard numbers: %w", err)
	}

	// 2. 创建新分镜
	newStoryboard := &models.Storyboard{
		EpisodeID:        uint(episodeID),
		StoryboardNumber: insertAfter + 1,
		Duration:         duration,
		Status:           "pending",
	}

	if err := tx.Create(newStoryboard).Error; err != nil {
		tx.Rollback()
	...
	if err := s.db.Preload("Background").Preload("Characters").
		First(newStoryboard, newStoryboard.ID).Error; err != nil {
		return nil, fmt.Errorf("failed to reload storyboard: %w", err)
	}

	s.log.Infow("Storyboard inserted",
		"episode_id", episodeID,
		"insert_after", insertAfter,
		"new_id", newStoryboard.ID,
		"new_number", newStoryboard.StoryboardNumber)
