...
func (s *StoryboardService) UpdateStoryboard(storyboardID string, updates map[string]interface{}) error {
	...
	if val, ok := updates["video_prompt"].(string); ok {
		// Allow explicit overrides, including empty string.
		updateData["video_prompt"] = val
		updateData["video_prompt_is_custom"] = true
		videoPromptProvided = true
	}
	...
	// video_prompt handling
	// - If user explicitly provides video_prompt, persist it and mark as custom.
	// - Else, if an existing custom prompt is present, keep it (do not overwrite).
	// - Else, regenerate based on current storyboard fields.
	if !videoPromptProvided {
		if storyboard.VideoPromptIsCustom {
			// Keep current custom prompt.
		} else {
			// image_prompt not auto-updated because it may correspond to multiple generated frames.
			videoPrompt := s.generateVideoPrompt(sb)
			updateData["video_prompt"] = videoPrompt
			updateData["video_prompt_is_custom"] = false
		}
	}
