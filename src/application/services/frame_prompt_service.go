...
type GenerateFramePromptRequest struct {
	...
	Style      string `json:"style,omitempty"`       // 提示词风格（可选）
}
...
func (s *FramePromptService) GenerateFramePrompt(req GenerateFramePromptRequest, model string) (*FramePromptResponse, error) {
	...
	case FrameTypeFirst:
		response.SingleFrame = s.generateFirstFrame(storyboard, scene, req.Style, model)
		// 保存单帧提示词
		s.saveFramePrompt(req.StoryboardID, string(req.FrameType), response.SingleFrame.Prompt, response.SingleFrame.Description, "")
	case FrameTypeKey:
		response.SingleFrame = s.generateKeyFrame(storyboard, scene, req.Style, model)
		s.saveFramePrompt(req.StoryboardID, string(req.FrameType), response.SingleFrame.Prompt, response.SingleFrame.Description, "")
	case FrameTypeLast:
		response.SingleFrame = s.generateLastFrame(storyboard, scene, req.Style, model)
		s.saveFramePrompt(req.StoryboardID, string(req.FrameType), response.SingleFrame.Prompt, response.SingleFrame.Description, "")
	case FrameTypePanel:
		...
		response.MultiFrame = s.generatePanelFrames(storyboard, scene, req.Style, count, model)
		// 保存多帧提示词（合并为一条记录）
		var prompts []string
	...
	case FrameTypeAction:
		response.MultiFrame = s.generateActionSequence(storyboard, scene, req.Style, model)
		var prompts []string
		for _, frame := range response.MultiFrame.Frames {
			prompts = append(prompts, frame.Prompt)
		}
...
func (s *FramePromptService) withStyleSystemPrompt(systemPrompt, style string) string {
	style = strings.TrimSpace(style)
	if style == "" {
		return systemPrompt
	}
	return systemPrompt + "\n\n【风格要求】\n- 生成的 prompt 必须体现以下风格：" + style + "\n- 不要单独解释风格，把风格自然融入画面描述中"
}

// generateFirstFrame 生成首帧提示词
func (s *FramePromptService) generateFirstFrame(sb models.Storyboard, scene *models.Scene, style string, model string) *SingleFramePrompt {
	// 构建上下文信息
	contextInfo := s.buildStoryboardContext(sb, scene)

	// 使用国际化提示词
	systemPrompt := s.withStyleSystemPrompt(s.promptI18n.GetFirstFramePrompt(), style)
	...
	if err != nil {
		...
		// 降级方案：使用简单拼接
		fallbackPrompt := s.buildFallbackPrompt(sb, scene, style, "first frame, static shot")
...
func (s *FramePromptService) generateKeyFrame(sb models.Storyboard, scene *models.Scene, style string, model string) *SingleFramePrompt {
	// 构建上下文信息
	contextInfo := s.buildStoryboardContext(sb, scene)

	// 使用国际化提示词
	systemPrompt := s.withStyleSystemPrompt(s.promptI18n.GetKeyFramePrompt(), style)
	...
	if err != nil {
		s.log.Warnw("AI generation failed, using fallback", "error", err)
		fallbackPrompt := s.buildFallbackPrompt(sb, scene, style, "key frame, dynamic action")
		return &SingleFramePrompt{
			Prompt:      fallbackPrompt,
...
func (s *FramePromptService) generateLastFrame(sb models.Storyboard, scene *models.Scene, style string, model string) *SingleFramePrompt {
	// 构建上下文信息
	contextInfo := s.buildStoryboardContext(sb, scene)

	// 使用国际化提示词
	systemPrompt := s.withStyleSystemPrompt(s.promptI18n.GetLastFramePrompt(), style)
	...
	if err != nil {
		s.log.Warnw("AI generation failed, using fallback", "error", err)
		fallbackPrompt := s.buildFallbackPrompt(sb, scene, style, "last frame, final state")
		return &SingleFramePrompt{
			Prompt:      fallbackPrompt,
...
func (s *FramePromptService) generatePanelFrames(sb models.Storyboard, scene *models.Scene, style string, count int, model string) *MultiFramePrompt {
	...
	// 固定生成：首帧 -> 关键帧 -> 尾帧
	if count == 3 {
		frames[0] = *s.generateFirstFrame(sb, scene, style, model)
		frames[0].Description = "第1格：初始状态"

		frames[1] = *s.generateKeyFrame(sb, scene, style, model)
		frames[1].Description = "第2格：动作高潮"

		frames[2] = *s.generateLastFrame(sb, scene, style, model)
		frames[2].Description = "第3格：最终状态"
	} else if count == 4 {
		// 4格：首帧 -> 中间帧1 -> 中间帧2 -> 尾帧
		frames[0] = *s.generateFirstFrame(sb, scene, style, model)
		frames[1] = *s.generateKeyFrame(sb, scene, style, model)
		frames[2] = *s.generateKeyFrame(sb, scene, style, model)
		frames[3] = *s.generateLastFrame(sb, scene, style, model)
	}
...
func (s *FramePromptService) generateActionSequence(sb models.Storyboard, scene *models.Scene, style string, model string) *MultiFramePrompt {
	// 将动作分解为5个步骤
	...
	frames[1] = *s.generateKeyFrame(sb, scene, style, model)
	frames[2] = *s.generateKeyFrame(sb, scene, style, model)
	frames[3] = *s.generateKeyFrame(sb, scene, style, model)
	frames[4] = *s.generateLastFrame(sb, scene, style, model)
...
// UpdateFramePrompt 更新帧提示词
func (s *FramePromptService) UpdateFramePrompt(storyboardID, frameType, prompt string) error {
	// 查找现有记录
	var framePrompt models.FramePrompt
	err := s.db.Where("storyboard_id = ? AND frame_type = ?", storyboardID, frameType).First(&framePrompt).Error
	...
	if err != nil {
		...
		return s.db.Create(&framePrompt).Error
	}

	// 更新现有记录
	framePrompt.Prompt = prompt
	return s.db.Save(&framePrompt).Error
}

// buildFallbackPrompt 构建降级提示词（AI失败时使用）
func (s *FramePromptService) buildFallbackPrompt(sb models.Storyboard, scene *models.Scene, style, suffix string) string {
	var parts []string
